...
public class ColorUtil {
    ...
    public Color getRainbow(int speed, int sat, int bright) {
        this.hue += (float) speed / 4000.0F;
        return Color.getHSBColor(this.hue, (float) sat / 255.0F, (float) bright / 255.0F);
    }

    private static int HUEtoRGB(float[] hue, int saturation, int brightness) {
        int rgb = Color.HSBtoRGB(hue[0], (float) saturation, (float) brightness);
        int red = rgb >> 16 & 255;
        int green = rgb >> 8 & 255;
        int blue = rgb & 255;
    ...
    public static int toRGBA(float r, float g, float b, float a) {
        return toRGBA((int) r, (int) g, (int) b, (int) a);
    }
    ...
    public String getColorNameFromRgb(int r, int g, int b) {
        ArrayList colorList = this.initColorList();
        ColorUtil.ColorName closestMatch = null;
        int minMSE = Integer.MAX_VALUE;
        Iterator iterator = colorList.iterator();

        while (iterator.hasNext()) {
            ColorUtil.ColorName c = (ColorUtil.ColorName) iterator.next();
    ...
    public static int colorToHex(Color c) {
        return Integer.decode("0x" + Integer.toHexString(c.getRGB()).substring(2)).intValue();
    }
