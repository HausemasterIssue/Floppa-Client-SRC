...
public class IntSlider extends PartTemplate {
    ...
    public IntSlider(Setting value, Button button, int offset) {
        this.setting = value;
        this.parent = button;
    ...
    public void updateComponent(int mouseX, int mouseY) {
        ...
        double min = (double) ((Integer) this.setting.min).intValue();
        double max = (double) ((Integer) this.setting.max).intValue();

        this.renderWidth = 88.0D * ((double) ((Integer) this.setting.getValue()).intValue() - min) / (max - min);
        if (this.dragging) {
            if (diff == 0.0D) {
                this.setting.setValue(this.setting.min);
            } else {
                DecimalFormat format = new DecimalFormat("##");
                String newValueInt = format.format(diff / 88.0D * (max - min) + min);

                this.setting.setValue(Integer.valueOf(Integer.parseInt(newValueInt)));
            }
        }
