...
public class DoubleSlider extends PartTemplate {
    ...
    public DoubleSlider(Setting value, Button button, int offset) {
        this.setting = value;
        this.parent = button;
    ...
    public void updateComponent(int mouseX, int mouseY) {
        ...
        double min = ((Double) this.setting.min).doubleValue();
        double max = ((Double) this.setting.max).doubleValue();

        this.renderWidth = 88.0D * (((Double) this.setting.getValue()).doubleValue() - min) / (max - min);
        if (this.dragging) {
            if (diff == 0.0D) {
                this.setting.setValue(this.setting.min);
            } else {
                DecimalFormat format = new DecimalFormat("##.0");
                String newValueDouble = format.format(diff / 88.0D * (max - min) + min);

                this.setting.setValue(Double.valueOf(Double.parseDouble(newValueDouble)));
            }
        }
